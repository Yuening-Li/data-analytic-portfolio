    ...
    def extractor(self, document):
        ...
        if self.cfg.bow:
           ...
           # feats = dict(feats.items() + bow_features.items())
            feats = dict([ [k,v] for k,v in feats.items() ] + [ [k,v] for k,v in bow_features.items() ])
        ...
        if self.cfg.bigram:
            ...
            feats = dict([ [k,v] for k,v in feats.items() ] + [ [k,v] for k,v in bigram_features.items() ])
...
class ReviewClassifier (BaseReviewClassifier):
    def __init__(self, csv_prefix, labels, pos_tbl_prefix, neg_tbl_prefix, classifier_cfg, cfg_id, init_trainset, old_traintest_set = False, datafile_suffix="_all"):
        ...
        train_size = int(round(data_size - 384)) #remove our test data
        #test_size = int(round(data_size))
        ...
        for i in range(num_folds):
            ...
            self.test = self.data_with_label[-train_size:]
        ...
        for label in labels:
            ...
            with open(filename, "a+") as csvfile:
                ...
                row_dict_final = dict ([ [k,v] for k,v in {"cfg_id" : cfg_id}.items() ] + [ [k,v] for k,v in row_dict_techniques.items() ] + [ [k,v] for k,v in row_dict_means.items()])

                
                writer.writerow(row_dict_final)

            filename = '%sprecision_TEST.csv' % csv_prefix_tmp
            p = pathlib.Path(filename)
            writeheader = not p.exists()
            ...
            with open(filename, "a+") as csvfile:
                ...
                row_dict_final = dict( [ [k,v] for k,v in {"cfg_id" : cfg_id}.items() ] + [ [k,v] for k,v in row_dict_techniques.items()] + [ [k,v] for k,v in row_dict_folds.items()])
                writer.writerow(row_dict_final)


            filename = '%srecall_TEST.csv' % csv_prefix_tmp
            p = pathlib.Path(filename)
            writeheader = not p.exists()
            ...
            filename = '%sf1score_TEST.csv' % csv_prefix_tmp
            p = pathlib.Path(filename)
            writeheader = not p.exists()
        ...
        filename = '%stime_TEST.csv' % csv_prefix
        p = pathlib.Path(filename)
        writeheader = not p.exists()
